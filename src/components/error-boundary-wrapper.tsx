...
export function ErrorBoundaryWrapper({
  ...
  useEffect(() => {
    const handleError = (event: ErrorEvent) => {
      ...
      if (event.error?.message?.includes('useContext') || event.error?.message?.includes('hook')) {
        ...
        document.cookie = '__Host-next-auth.csrf-token=; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT';
