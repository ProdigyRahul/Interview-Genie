...
export async function GET(
  ...
  try {
    ...
    if (!resume.personalInfo) {
      return NextResponse.json({ error: "Resume data is incomplete" }, { status: 400 });
    }
    ...
    try {
      ...
      return new NextResponse(pdfBuffer, {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="${resume.personalInfo.fullName.replace(/[^a-zA-Z0-9]/g, '-')}-Resume.pdf"`.toLowerCase(),
        },
      });
    } catch (pdfError) {
